...
# This code fixes weird import errors I had from depreciated packages
np_load_old = np.load
...
def createMatrix(conversationDictionary, wList, maxLen):
...
def createAllSets(conversationFileName):
    conversationDictionary = np.load(conversationFileName).item()
    numExamples = len(conversationDictionary)
    trainDictSize = math.floor(numExamples * .7)
    trainDict = {}
    validationDictSize = math.floor(numExamples * .15)
    validationDict = {}
    testDictSize = math.floor(numExamples * .15)
    testDict = {}
    # This fills the training set with 70% of data from original dictionary
    for item in range(trainDictSize):
        random_item = key, val = random.choice(list(conversationDictionary.items()))
        popped_value = {key: conversationDictionary.pop(key)}
        trainDict.update(popped_value)
        # This fills the validation set with 15% of data from original dictionary
    for item in range(validationDictSize):
        random_item = key, val = random.choice(list(conversationDictionary.items()))
        popped_value = {key: conversationDictionary.pop(key)}
        validationDict.update(popped_value)
    # This fills the test set with 15% of data from original dictionary
    for item in range(testDictSize):
        random_item = key, val = random.choice(list(conversationDictionary.items()))
        popped_value = {key: conversationDictionary.pop(key)}
        testDict.update(popped_value)
    return trainDict, validationDict, testDict
    #numExamples, xTrain, yTrain = createTrainingMatrices()
...
#else:
trainingSet, validationSet, testSet = createAllSets('conversationDictionary.npy')
numTrainingExamples, xTrain, yTrain = createMatrix(trainingSet, wordList, maxEncoderLength)
numValidationExamples, xValidate, yValidate = createMatrix(validationSet, wordList, maxEncoderLength)
numTestExamples, xTest, yTest = createMatrix(testSet, wordList, maxEncoderLength)

np.save('Seq2SeqXTrain.npy', xTrain)
np.save('Seq2SeqYTrain.npy', yTrain)
np.save('Seq2SeqXValidate.npy', xValidate)
np.save('Seq2SeqYValidate.npy', yValidate)
np.save('Seq2SeqXTest.npy', xTest)
np.save('Seq2SeqYTest.npy', yTest)
print('Finished creating all matrices')
...
# This saves the model into something we can use later
